...
HTMLTreeBuilder::HTMLTreeBuilder(HTMLToken& t_token)
    : m_token{ t_token }
{}


void HTMLTreeBuilder::receive_character_token(char32_t character)
{
    std::println("TOKEN: Character");
    char char_buf[5];
    UTF32_Util::char_utf32_to_utf8(character, char_buf);
    std::println("Data: {}", char_buf);
}


void HTMLTreeBuilder::receive_token()
{
    /******************************************************************/
    ...
    case HTMLToken::Type::DOCTYPE:
        std::println("DOCTYPE");
        std::println("Force quirks: {}", m_token.m_doctype_data->m_force_quirks);
        // NAME
        std::println("Name missing: {}", m_token.m_doctype_data->m_name_missing);
        std::print("Name: ");
        print_char32_array(m_token.m_data);
        // PUBLIC IDENTIFIER
        std::println("Public identifier missing: {}", m_token.m_doctype_data->m_public_identifier_missing);
        std::print("Public identifier: ");
        print_char32_array(m_token.m_doctype_data->m_public_identifier);
        // SYSTEM IDENTIFIER
        std::println("System identifier missing: {}", m_token.m_doctype_data->m_system_identifier_missing);
        std::print("System identifier: ");
        print_char32_array(m_token.m_doctype_data->m_system_identifier);
        break;

    case HTMLToken::Type::StartTag:
    case HTMLToken::Type::EndTag:
        std::println("{}", (m_token.m_type == HTMLToken::Type::StartTag) ? "Start Tag" : "End Tag" );
        std::print("Tag name: ");
        print_char32_array(m_token.m_data);
        std::println("Self closing: {}", m_token.m_self_closing);
        std::println("Attributes:");
        for (auto attr : m_token.m_attributes) {
